...
static LLRegisterWidget<LLHexEditor> r("hex_editor");

LLHexEditor::LLHexEditor(const std::string& name, const LLRect& rect)
...
LLView* LLHexEditor::fromXML(LLXMLNodePtr node, LLView *parent, class LLUICtrlFactory *factory)
{
	...
	LLHexEditor* editor = new LLHexEditor(name, rect);
...
void LLHexEditor::getPosAndContext(S32 x, S32 y, BOOL force_context, U32& pos, BOOL& in_data, BOOL& second_nibble)
{
	pos = 0;
...
U32 LLHexEditor::getProperSelectionStart()
{
	return (mSelectionStart < mSelectionEnd) ? mSelectionStart : mSelectionEnd;
}

U32 LLHexEditor::getProperSelectionEnd()
{
