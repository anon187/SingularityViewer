...
void LLKeyTool::openKey(LLUUID id, LLKeyType key_type, LLAssetType::EType asset_type)
{
	...
	if(key_type == LLKeyTool::KT_ASSET)
	{
		...
		{
		LLWearableList::getInstance()->getAsset(id, name, asset_type, wearable_callback, (void*)name.c_str());
		}
		else if(asset_type != LLAssetType::AT_BODYPART || asset_type != LLAssetType::AT_CLOTHING)
		{
