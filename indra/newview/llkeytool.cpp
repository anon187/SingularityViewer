...
LLKeyTool::~LLKeyTool()
{
	...
	if(mObjectPropertiesFamilyRequests >= sObjectPropertiesFamilyRequests)
	{
		...
		mObjectPropertiesFamilyRequests = 0;
		gMessageSystem->setHandlerFuncFast(_PREHASH_ObjectPropertiesFamily, &onObjectPropertiesFamily);
	}
	if(mParcelInfoRequests >= sParcelInfoRequests)
	{
		sParcelInfoRequests = 0;
		mParcelInfoRequests = 0;
		gMessageSystem->setHandlerFuncFast(_PREHASH_ParcelInfoReply, &onParcelInfoReply);
	}
	if(mImageRequests >= sImageRequests)
	{
		sImageRequests = 0;
		mImageRequests = 0;
		gMessageSystem->setHandlerFuncFast(_PREHASH_ImageData, &onImageData);
		gMessageSystem->setHandlerFuncFast(_PREHASH_ImageNotInDatabase, &onImageNotInDatabase);
	}
	if(mTransferRequests >= sTransferRequests)
	{
		sTransferRequests = 0;
		mTransferRequests = 0;
		gMessageSystem->setHandlerFuncFast(_PREHASH_TransferInfo, &onTransferInfo);
	}
	// Remove from map
